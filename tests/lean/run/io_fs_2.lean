...
do rmdir TEST_DIR,
   ncheck "dir exists now" $ exist TEST_DIR,
   ycheck "mkdir" $ mkdir TEST_DIR,
   ycheck "dir exists now" $ exist TEST_DIR,
...
do let combined := TEST_DIR1 ++ "/" ++ TEST_DIR2,
   ...
   ycheck "dir exists now1" $ exist TEST_DIR1,
   ycheck "dir exists now2" $ exist combined,
   ycheck "rmdir2" $ rmdir $ combined,
   ycheck "dir exists now1'" $ exist TEST_DIR1,
   ncheck "dir exists now2'" $ exist combined,
   ycheck "rmdir" $ rmdir $ TEST_DIR1,
   ncheck "dir exists now1''" $ exist TEST_DIR1,
   ncheck "dir exists now2''" $ exist combined,
   return ()
...
do remove TEST_FILE_1 <|> return (),
   ...
   ncheck "file1 exists now" $ exist TEST_FILE_1,
   mk_file_handle TEST_FILE_1 io.mode.write >>= close,
   ycheck "file1 exists now'" $ exist TEST_FILE_1,
   rename TEST_FILE_1 TEST_FILE_2,
   ncheck "file1 exists now''" $ exist TEST_FILE_1,
   ycheck "file2 exists now" $ exist TEST_FILE_2,
   remove TEST_FILE_2,
   ncheck "file2 exists now" $ exist TEST_FILE_2,
   return ()
