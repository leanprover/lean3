...
//    ios().get_diagnostic_channel() << "FUSE: " << e << "\n\n";
    ...
    */
    ...
    auto pf_1_3 = prove(get_app_builder().mk_eq(e, e_grp),
                        mk_simp_rule_sets(env(), ios().get_options(),
    ...
    result r_simp_ls = simplify(e_fused_ls, mk_simp_rule_sets(env(), ios().get_options(),
...
void initialize_simplifier() {
    ...
    register_trace_class(name({"simplifier", "rules"}));
...
result som_fuse(expr const & e) {
    simp_rule_sets srss = mk_simp_rule_sets(env(), ios().get_options(),
                                             {*g_simplify_prove_namespace, *g_simplify_unit_namespace,
                                                     *g_simplify_neg_namespace, *g_simplify_ac_namespace,
                                                     *g_simplify_distrib_namespace});
