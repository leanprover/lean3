...
/* Try to use congruence lemmas generated by the congr_lemma module.
   ...
   \remark has_extra_args will be set to true if e is an application
   with more arguments than the congruence lemma expects.
*/
optional<simp_result> simplify_core_fn::try_auto_eq_congr(expr const & e, bool & has_extra_args) {
    ...
    has_extra_args = (args.size() > new_args.size());
    ...
    if (!has_proof) {
        simp_result r = simp_result(mk_app(mk_app(f, new_args),
                                           args.size() - new_args.size(), args.data() + new_args.size()));
        return optional<simp_result>(r);
    }
    ...
    for (unsigned arg_idx = new_args.size(); arg_idx < args.size(); ++arg_idx) {
        r = congr_fun(r, args[arg_idx]);
    }
...
simp_result simplify_core_fn::visit_app(expr const & _e) {
    ...
    if (m_rel == get_eq_name()) {
        ...
        optional<simp_result> r_args = try_auto_eq_congr(e, has_extra_args);
        if (r_args && !has_extra_args)
            return join(*r_args, visit_fn(r_args->get_new()));
