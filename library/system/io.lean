...
definition to_string_nat : nat -> string
  | to_string_nat nat.zero := "0"
  | to_string_nat (nat.succ n) := string_append "1 + " (to_string_nat n)

definition ToString_nat [instance] : ToString nat :=
  {| ToString,
     to_string := to_string_nat |}
